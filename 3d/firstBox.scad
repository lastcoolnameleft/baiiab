...
wall_thickness = (1/15.5) * width_outer;
echo(wall_thickness);
...
printer_inset_x = 102.70;
printer_inset_y = 57;

printer_wing_x = 2.5;
printer_wing_y = 30;

button_radius = 28 / 2; // buttons are 24mm wide
...
module printer_hole() {
    translate([0,23,0])
        cube([printer_inset_x, printer_inset_y, lid_thickness], center=true);
}

module button_hole() {
    translate([0,-40,0])
        cylinder(lid_thickness*2, r= button_radius, center=true);
}
